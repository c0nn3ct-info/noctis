...
<p align="center">
  ...
  <a href="./site/LICENSE.md"><img src="https://img.shields.io/badge/license-EULA-blue" alt="许可：EULA"></a>
  <a href="https://github.com/c0nn3ct-info/noctis"><img src="https://img.shields.io/badge/helper-MIT-green" alt="Helper: MIT"></a>
  <a href="https://noctis.c0nn3ct.info"><img src="https://img.shields.io/badge/site-noctis.c0nn3ct.info-7c3aed" alt="站点"></a>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="./site/media/demo-zh-dark.gif">
    <img alt="Noctis demo" src="./site/media/demo-zh-light.gif" width="720">
  </picture>
</p>
...
- **备份与迁移** — 服务器、订阅、配置和设置可导出为一个 JSON 文件，也能从该文件导入回来。单个路由配置还能单独存成文件，规则集因此可以在浏览器和机器之间搬运，无需重新录入。
- **WebRTC 泄漏防护** — 可选开关，阻止代理之外的 UDP，让 WebRTC 无法暴露你的真实 IP。
- **一条规则拦截广告和追踪器** — `category-ads-all` 系列就在 Geosite 字段的建议里。把它加进 Block 分组，配置就会丢弃广告和追踪流量。
...
Block 丢弃流量，而不是转发流量：广告和跟踪域名、遥测上报地址、不希望在这个浏览器里打开的站点。新建的配置是空的，Noctis 不会自己加规则。想拦掉广告，把 `category-ads-all` 系列加进 Block，Geosite 字段会把它列在建议里。如果某个页面被意外拦截，拦截页面会写出配置名和规则，并可以直接删除它。
