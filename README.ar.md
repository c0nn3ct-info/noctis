...
<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="./site/media/demo-ar-dark.gif">
    <img alt="Noctis demo" src="./site/media/demo-ar-light.gif" width="720">
  </picture>
</p>
...
- **النسخ الاحتياطي والنقل** — صدّر الخوادم والاشتراكات وملفات التعريف والإعدادات إلى ملف JSON واحد، ثم استورد الملف نفسه من جديد. ولكل ملف تعريف توجيه ملفه الخاص أيضًا، فتنتقل مجموعة القواعد بين المتصفحات والأجهزة دون إعادة كتابتها.
- **حماية من تسريب WebRTC** — مفتاح اختياري يحظر UDP خارج الوكيل كي لا يكشف WebRTC عنوان IP الحقيقي الخاص بك.
- **حظر الإعلانات والمتعقّبات بقاعدة واحدة** — عائلة `category-ads-all` موجودة ضمن اقتراحات حقل Geosite. أضِفها إلى مجموعة Block ليُسقط الملف الإعلانات والمتعقّبات.
...
ملف التوجيه هو ثلاث مجموعات من القواعد وترتيب فحصها. تُظهر شرائح **Evaluation order** في شاشة الملف هذا الترتيب، ويمكن سحبها لإعادة ترتيبها.

| المجموعة | ما يحدث للمطابقة |
| --- | --- |
| **Direct** | يخرج عبر اتصالك الحقيقي متجاوزًا الوكيل. |
| **Proxy** | يخرج عبر السيرفر النشط. |
| **Block** | يُسقط الطلب. يعرض Noctis صفحة الحجب الخاصة به ويذكر القاعدة التي طابقت. |

المطابقة الأولى هي الفائزة، لذا يحسم الترتيب الحالات المتعارضة. بالترتيب الافتراضي `Direct → Proxy → Block` يخرج المضيف المذكور في Direct وBlock معًا اتصالًا مباشرًا.

تقبل كل مجموعة ثلاثة أنواع من المدخلات: **Domains** (`example.com`، `*.example.com`)، و**Geosite** (اسم فئة مثل `youtube` أو `ads`)، و**Geoip** (رمز دولة مثل `cn` أو نطاق CIDR).

ما لا يطابق أي مجموعة يخرج مباشرة. المجموعات تعمل في وضع Rules فقط: Global يمرر كل الترافيك عبر الوكيل، وDirect يتجاهل الوكيل تمامًا.

### متى تضيف شيئًا إلى Block

Block يسقط الترافيك بدل توجيهه: نطاقات الإعلانات والتتبع، ونقاط إرسال القياسات، وموقع لا تريد فتحه في هذا المتصفح. الملف الجديد يبدأ فارغًا، لأن Noctis لا يضيف قواعد من عنده. ولإسقاط الإعلانات أضِف عائلة `category-ads-all` إلى Block، وحقل Geosite يقترحها. وإذا حُجبت صفحة على غير المتوقع، تذكر صفحة الحجب اسم الملف والقاعدة وتتيح حذفها من هناك.
...
**ماذا يفعل قسم Block في ملف التوجيه؟**
يسقط Block الطلبات المطابقة بدل إرسالها إلى أي مكان: لا وكيل ولا اتصال مباشر. يعرض Noctis صفحة الحجب الخاصة به ويذكر القاعدة التي طابقت. السلسلة الكاملة في [قواعد التوجيه](#-قواعد-التوجيه).
