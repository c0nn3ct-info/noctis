...
- **Бэкап и перенос** — Серверы, подписки, профили и настройки выгружаются в один файл JSON и загружаются из него обратно. Отдельный профиль маршрутизации сохраняется своим файлом, поэтому набор правил переезжает между браузерами и машинами без повторного ввода.
- **Защита от утечек WebRTC** — Опциональный тумблер блокирует UDP мимо прокси, чтобы WebRTC не раскрыл ваш реальный IP.
- **Блокировка рекламы и трекеров одним правилом** — Семейство `category-ads-all` лежит в подсказках поля Geosite. Добавьте его в группу Block, и профиль начнёт отбрасывать рекламу и трекеры.
...
Block сбрасывает трафик вместо того, чтобы его маршрутизировать: домены рекламы и трекеров, точки сбора телеметрии, сайт, который не должен открываться в этом браузере. Новый профиль пуст: своих правил Noctis не добавляет. Чтобы убрать рекламу, добавьте в Block семейство `category-ads-all`, поле Geosite предлагает его в подсказках. Если страница оказалась заблокирована неожиданно, на странице блокировки указаны профиль и правило, и оттуда правило можно удалить.
