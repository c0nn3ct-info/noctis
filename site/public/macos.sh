...
# One entry per <UserDataDir>/NativeMessagingHosts. A channel missing from this
# list is a browser where the helper installs fine and the extension then
# reports it as not found, so every channel the site advertises belongs here.
TARGETS=(
  ...
  "$HOME/Library/Application Support/Vivaldi Snapshot/NativeMessagingHosts"
